...
export default function AttendancePage() {
  ...
  const [classRecords, setClassRecords] = useState<AcademicRecord[]>([]);
  ...
  useEffect(() => {
    academicService
      ...
      .then((data) => setClassRecords(Array.isArray(data) ? (data as AcademicRecord[]) : []))
      .catch(() => undefined);
  }, [studentId]);
  return <><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><PageHeader eyebrow="Journey record" title="Attendance" description="Review both class and bus pickup records." /><StudentSelector onChange={selectStudent} /></div>
    <Card className="mb-5 p-5"><p className="font-display text-lg font-bold">Class attendance</p><p className="mt-1 text-sm text-[var(--muted-foreground)]">{classRecords.length ? `${classRecords.length} classroom record${classRecords.length === 1 ? "" : "s"} available.` : "No classroom attendance records yet."}</p></Card>
