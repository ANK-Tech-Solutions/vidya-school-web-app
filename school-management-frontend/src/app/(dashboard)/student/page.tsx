...
import { BusFront, Clock3, MapPin, Navigation, ShieldCheck, UserRound } from "lucide-react";
...
export default function StudentPage() {
  ...
  const cards = [
    ...
    { label: "ETA", value: dashboard?.eta ?? trip?.eta ?? "Not available", icon: Clock3, color: "text-emerald-500" },
  ];
  ...
  return <><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><PageHeader eyebrow="Your journey" title="Travel with confidence." description="Keep track of every important part of today’s bus journey." /><StudentSelector onChange={selectStudent} /></div>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2"><Card className="p-6"><ShieldCheck className="text-emerald-500" size={23} /><p className="mt-5 text-sm text-[var(--muted-foreground)]">Trip status</p><h2 className="mt-1 font-display text-2xl font-bold">{trip?.status?.replaceAll("_", " ") ?? "No active trip"}</h2><p className="mt-2 text-sm text-[var(--muted-foreground)]">Pickup time: {dashboard?.pickupTime ?? trip?.pickupTime ?? dashboard?.route?.pickupTime ?? "To be confirmed"}</p></Card>
      <Card className="p-6"><h2 className="font-display text-xl font-bold">Quick links</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{[{ href: "/student/tracking", label: "Track bus", icon: Navigation }, { href: "/student/attendance", label: "Attendance", icon: ShieldCheck }].map(({ href, label, icon: Icon }) => <Link key={href} href={href} className="flex items-center gap-3 rounded-xl border border-[var(--border)] p-4 text-sm font-semibold hover:bg-[var(--muted)]"><Icon size={18} className="text-[var(--primary)]" />{label}</Link>)}</div></Card></div>
  </>;
}
